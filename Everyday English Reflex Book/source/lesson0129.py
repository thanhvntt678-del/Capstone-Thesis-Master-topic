# -*- coding: utf-8 -*-
LESSON_0129 = {
    'lesson_id': '0129',
    'cefr': 'A0 / Pre-A1',
    'domain': 'Numbers and Time',
    'en_title': 'Asking and Answering Very Simple Questions About a Phone Number',
    'vi_title': 'Giao tiếp nền tảng: numbers, dates & time — a phone number',
    'intro_en': (
        "Ms Lan and Mr Leopold meet in many everyday situations where a phone number needs to be asked "
        "for or given -- at a clinic front desk, a delivery booking, a taxi app, a parent's contact form, "
        "a job application, a gym sign-up, a restaurant reservation, a repair shop ticket, a school "
        "emergency contact form, a bank account form, a hotel check-in, a hair salon booking, a lost-"
        "property report, a neighbourhood contact list, a community class sign-up, a pharmacy account, a "
        "utility company setup, a pet clinic file, a wedding invitation RSVP, a charity donation receipt, "
        "a co-working space registration, a car repair booking, a library membership, a fitness class "
        "waiver, a moving company quote, a photography booking, a tailor's order slip, a dentist "
        "appointment, a volunteer sign-up sheet, a parcel delivery form, a subscription account, an "
        "insurance policy form, a driving lesson booking, a rental application, a conference registration, "
        "and a final wrap-up about why sharing a phone number clearly matters -- in every scene, one of "
        "them asks a very simple question about a phone number, and the other answers clearly, giving the "
        "number digit by digit."
    ),
    'intro_vi': (
        "Chị Lan và anh Leopold gặp nhau trong nhiều tình huống hằng ngày cần hỏi hoặc cho biết số điện "
        "thoại -- tại quầy lễ tân phòng khám, đặt giao hàng, ứng dụng đặt taxi, mẫu đơn liên hệ phụ "
        "huynh, đơn xin việc, đăng ký phòng gym, đặt bàn nhà hàng, phiếu sửa chữa, mẫu đơn liên hệ khẩn "
        "cấp của trường học, mẫu đơn mở tài khoản ngân hàng, nhận phòng khách sạn, đặt lịch tiệm tóc, báo "
        "cáo đồ thất lạc, danh sách liên hệ khu phố, đăng ký lớp học cộng đồng, tài khoản nhà thuốc, đăng "
        "ký công ty tiện ích, hồ sơ phòng khám thú y, xác nhận tham dự đám cưới, biên nhận quyên góp từ "
        "thiện, đăng ký không gian làm việc chung, đặt lịch sửa xe, thẻ thành viên thư viện, mẫu miễn trừ "
        "lớp thể dục, báo giá công ty chuyển nhà, đặt lịch chụp ảnh, phiếu đặt hàng ở tiệm may, cuộc hẹn "
        "nha khoa, tờ đăng ký tình nguyện viên, mẫu đơn giao bưu kiện, tài khoản đăng ký gói dịch vụ, mẫu "
        "đơn hợp đồng bảo hiểm, đặt lịch học lái xe, đơn xin thuê nhà, đăng ký hội nghị, và cuối cùng là "
        "vì sao chia sẻ số điện thoại rõ ràng lại quan trọng -- trong mỗi tình huống, một người hỏi một "
        "câu hỏi rất đơn giản về số điện thoại, và người kia trả lời rõ ràng, đọc số từng chữ số một."
    ),
    'turns': [
        # 1 clinic front desk
        ("Ms Lan", "Could I get a good contact number for you?", "Cho tôi xin số điện thoại liên hệ của anh được không?"),
        ("Mr Leopold", "Of course, my number is zero nine one two three four five six seven.", "Được chứ, số của tôi là không chín một hai ba bốn năm sáu bảy."),
        ("Ms Lan", "Got it, jotting that number down now.", "Được rồi, tôi ghi số đó ngay."),
        # 2 delivery booking
        ("Mr Leopold", "What number should the delivery driver call if needed?", "Người giao hàng nên gọi số nào nếu cần vậy?"),
        ("Ms Lan", "He can reach me at zero nine two three four five six seven eight.", "Anh ấy liên hệ tôi qua số không chín hai ba bốn năm sáu bảy tám."),
        ("Mr Leopold", "Got it, that is saved for the driver.", "Được rồi, đã lưu cho tài xế."),
        # 3 taxi app
        ("Ms Lan", "What number is linked to your taxi app account?", "Số nào liên kết với tài khoản ứng dụng taxi của anh vậy?"),
        ("Mr Leopold", "It is zero nine three four five six seven eight nine.", "Là không chín ba bốn năm sáu bảy tám chín."),
        ("Ms Lan", "Still the same number, good, all matches up.", "Vẫn cùng một số, tốt, khớp hết rồi."),
        # 4 parent's contact form
        ("Mr Leopold", "What number should the school use to reach you in an emergency?", "Trường nên dùng số nào để liên hệ chị lúc khẩn cấp vậy?"),
        ("Ms Lan", "Please use zero nine four five six seven eight nine zero.", "Vui lòng dùng không chín bốn năm sáu bảy tám chín không."),
        ("Mr Leopold", "Noted, saved as your emergency contact.", "Ghi nhận rồi, đã lưu làm liên hệ khẩn cấp."),
        # 5 job application
        ("Ms Lan", "What is the best number to reach you for an interview?", "Số nào tốt nhất để liên hệ anh cho buổi phỏng vấn vậy?"),
        ("Mr Leopold", "You can call zero nine five six seven eight nine zero one anytime.", "Chị có thể gọi không chín năm sáu bảy tám chín không một bất cứ lúc nào."),
        ("Ms Lan", "Thank you, we will call you this week.", "Cảm ơn anh, tuần này chúng tôi sẽ gọi."),
        # 6 gym sign-up
        ("Mr Leopold", "What number should we put on your gym membership file?", "Nên ghi số nào vào hồ sơ thành viên phòng gym của chị vậy?"),
        ("Ms Lan", "Put down zero nine six seven eight nine zero one two, please.", "Ghi giúp tôi không chín sáu bảy tám chín không một hai nhé."),
        ("Mr Leopold", "Done, entered into the system now.", "Xong rồi, đã nhập vào hệ thống."),
        # 7 restaurant reservation
        ("Ms Lan", "Could I get a number in case we need to change the reservation?", "Cho tôi xin số phòng khi cần đổi đặt bàn được không?"),
        ("Mr Leopold", "Of course, it is zero nine seven eight nine zero one two three.", "Được chứ, là không chín bảy tám chín không một hai ba."),
        ("Ms Lan", "That number is logged, table confirmed now.", "Số đó đã ghi lại rồi, bàn xác nhận xong."),
        # 8 repair shop ticket
        ("Mr Leopold", "What number should we call once the repair is finished?", "Nên gọi số nào khi sửa xong vậy?"),
        ("Ms Lan", "Call zero nine eight nine zero one two three four, please.", "Vui lòng gọi không chín tám chín không một hai ba bốn."),
        ("Mr Leopold", "Thank you, that is on the ticket now.", "Cảm ơn chị, đã ghi vào phiếu rồi."),
        # 9 school emergency contact form
        ("Ms Lan", "What is your alternate number for the school records?", "Số dự phòng của anh cho hồ sơ trường là gì vậy?"),
        ("Mr Leopold", "My alternate number is zero nine nine zero one two three four five.", "Số dự phòng của tôi là không chín chín không một hai ba bốn năm."),
        ("Ms Lan", "Thank you, added as your backup contact.", "Cảm ơn anh, đã thêm làm liên hệ dự phòng."),
        # 10 bank account form
        ("Mr Leopold", "What number should we link to this new bank account?", "Nên liên kết số nào với tài khoản ngân hàng mới này vậy?"),
        ("Ms Lan", "Please link zero eight one two three four five six seven.", "Vui lòng liên kết không tám một hai ba bốn năm sáu bảy."),
        ("Mr Leopold", "Done, linked to your account now.", "Xong rồi, đã liên kết với tài khoản của chị."),
        # 11 hotel check-in
        ("Ms Lan", "Could I get a contact number for your reservation, please?", "Cho tôi xin số liên hệ cho đặt phòng của anh được không?"),
        ("Mr Leopold", "Sure, it is zero eight two three four five six seven eight.", "Được chứ, là không tám hai ba bốn năm sáu bảy tám."),
        ("Ms Lan", "Thank you, your room is ready.", "Cảm ơn anh, phòng của anh đã sẵn sàng."),
        # 12 hair salon booking
        ("Mr Leopold", "What number should we text to confirm your appointment?", "Nên nhắn tin xác nhận cuộc hẹn tới số nào vậy?"),
        ("Ms Lan", "Text zero eight three four five six seven eight nine, please.", "Vui lòng nhắn tới không tám ba bốn năm sáu bảy tám chín."),
        ("Mr Leopold", "Thank you, confirmation sent now.", "Cảm ơn chị, đã gửi xác nhận rồi."),
        # 13 lost-property report
        ("Ms Lan", "What number can we reach you at if we find your bag?", "Nếu tìm thấy túi của anh thì liên hệ số nào vậy?"),
        ("Mr Leopold", "Please call zero eight four five six seven eight nine zero.", "Vui lòng gọi không tám bốn năm sáu bảy tám chín không."),
        ("Ms Lan", "Thank you, we will call if it turns up.", "Cảm ơn anh, chúng tôi sẽ gọi nếu tìm thấy."),
        # 14 neighbourhood contact list
        ("Mr Leopold", "What number should go on our neighbourhood contact list for you?", "Nên ghi số nào vào danh sách liên hệ khu phố cho chị vậy?"),
        ("Ms Lan", "Put zero eight five six seven eight nine zero one, please.", "Vui lòng ghi không tám năm sáu bảy tám chín không một."),
        ("Mr Leopold", "Added, thank you for sharing it.", "Đã thêm, cảm ơn chị đã chia sẻ."),
        # 15 community class sign-up
        ("Ms Lan", "What number should I write on the class sign-up sheet?", "Tôi nên viết số nào vào tờ đăng ký lớp học vậy?"),
        ("Mr Leopold", "Write zero eight six seven eight nine zero one two, please.", "Vui lòng viết không tám sáu bảy tám chín không một hai."),
        ("Ms Lan", "Perfect, that number is now on the roster.", "Hoàn hảo, số đó đã có trong danh sách rồi."),
        # 16 pharmacy account
        ("Mr Leopold", "What number should be linked to your pharmacy account?", "Nên liên kết số nào với tài khoản nhà thuốc của chị vậy?"),
        ("Ms Lan", "Link zero eight seven eight nine zero one two three, please.", "Vui lòng liên kết không tám bảy tám chín không một hai ba."),
        ("Mr Leopold", "Done, your account is updated.", "Xong rồi, tài khoản của chị đã cập nhật."),
        # 17 utility company setup
        ("Ms Lan", "What number should we register for this utility account?", "Nên đăng ký số nào cho tài khoản tiện ích này vậy?"),
        ("Mr Leopold", "Register zero eight eight nine zero one two three four, please.", "Vui lòng đăng ký không tám tám chín không một hai ba bốn."),
        ("Ms Lan", "Great, the account is now set up with that number.", "Tốt, tài khoản đã tạo xong với số đó."),
        # 18 pet clinic file
        ("Mr Leopold", "What owner's number should go on this pet's file?", "Nên ghi số của chủ nào vào hồ sơ thú cưng này vậy?"),
        ("Ms Lan", "Put zero eight nine zero one two three four five, please.", "Vui lòng ghi không tám chín không một hai ba bốn năm."),
        ("Mr Leopold", "Added, the vet will call that number.", "Đã thêm, bác sĩ thú y sẽ gọi số đó."),
        # 19 wedding invitation RSVP
        ("Ms Lan", "What number should I put on this RSVP card?", "Tôi nên ghi số nào vào thẻ xác nhận tham dự này vậy?"),
        ("Mr Leopold", "Put zero seven one two three four five six seven, please.", "Vui lòng ghi không bảy một hai ba bốn năm sáu bảy."),
        ("Ms Lan", "Thank you, we will confirm your seat soon.", "Cảm ơn anh, chúng tôi sẽ xác nhận chỗ ngồi sớm."),
        # 20 charity donation receipt
        ("Mr Leopold", "What number should go on your donation receipt?", "Nên ghi số nào vào biên nhận quyên góp của chị vậy?"),
        ("Ms Lan", "Please use zero seven two three four five six seven eight.", "Vui lòng dùng không bảy hai ba bốn năm sáu bảy tám."),
        ("Mr Leopold", "Perfect, here is your receipt with that number.", "Hoàn hảo, đây là biên nhận có số đó."),
        # 21 co-working space registration
        ("Ms Lan", "What number should we use for your co-working space account?", "Nên dùng số nào cho tài khoản không gian làm việc chung của anh vậy?"),
        ("Mr Leopold", "Use zero seven three four five six seven eight nine, please.", "Vui lòng dùng không bảy ba bốn năm sáu bảy tám chín."),
        ("Ms Lan", "Noted, the desk booking is linked to that number.", "Ghi nhận rồi, đặt bàn liên kết với số đó."),
        # 22 car repair booking
        ("Mr Leopold", "What number should the garage call once your car is ready?", "Xưởng sửa xe nên gọi số nào khi xe chị xong vậy?"),
        ("Ms Lan", "Call zero seven four five six seven eight nine zero, please.", "Vui lòng gọi không bảy bốn năm sáu bảy tám chín không."),
        ("Mr Leopold", "Thank you, noted on the work order.", "Cảm ơn chị, đã ghi vào phiếu công việc."),
        # 23 library membership
        ("Ms Lan", "What number should I put on this library membership form?", "Tôi nên ghi số nào vào mẫu đơn thành viên thư viện này vậy?"),
        ("Mr Leopold", "Put zero seven five six seven eight nine zero one, please.", "Vui lòng ghi không bảy năm sáu bảy tám chín không một."),
        ("Ms Lan", "Got it, your library card will use that number.", "Được rồi, thẻ thư viện của anh sẽ dùng số đó."),
        # 24 fitness class waiver
        ("Mr Leopold", "What emergency number should be on this fitness waiver?", "Nên ghi số khẩn cấp nào vào mẫu miễn trừ lớp thể dục này vậy?"),
        ("Ms Lan", "Put zero seven six seven eight nine zero one two, please.", "Vui lòng ghi không bảy sáu bảy tám chín không một hai."),
        ("Mr Leopold", "Good, the waiver is filed under that number now.", "Tốt, mẫu miễn trừ đã lưu dưới số đó rồi."),
        # 25 moving company quote
        ("Ms Lan", "What number should the moving company call with a quote?", "Công ty chuyển nhà nên gọi số nào để báo giá vậy?"),
        ("Mr Leopold", "Call zero seven seven eight nine zero one two three, please.", "Vui lòng gọi không bảy bảy tám chín không một hai ba."),
        ("Ms Lan", "Thank you, they will call within the hour.", "Cảm ơn anh, họ sẽ gọi trong vòng một tiếng."),
        # 26 photography booking
        ("Mr Leopold", "What number should be on your photography booking form?", "Nên ghi số nào vào mẫu đặt lịch chụp ảnh của chị vậy?"),
        ("Ms Lan", "Put zero seven eight nine zero one two three four, please.", "Vui lòng ghi không bảy tám chín không một hai ba bốn."),
        ("Mr Leopold", "Thank you, your session is booked.", "Cảm ơn chị, buổi chụp đã được đặt."),
        # 27 tailor's order slip
        ("Ms Lan", "What number should we write on your order slip?", "Nên viết số nào vào phiếu đặt hàng của anh vậy?"),
        ("Mr Leopold", "Write zero seven nine zero one two three four five, please.", "Vui lòng viết không bảy chín không một hai ba bốn năm."),
        ("Ms Lan", "Great, that number is on the order slip now.", "Tốt, số đó đã ghi vào phiếu đặt hàng rồi."),
        # 28 dentist appointment
        ("Mr Leopold", "What number should we use to remind you of your appointment?", "Nên dùng số nào để nhắc chị về cuộc hẹn vậy?"),
        ("Ms Lan", "Use zero six one two three four five six seven, please.", "Vui lòng dùng không sáu một hai ba bốn năm sáu bảy."),
        ("Mr Leopold", "Thank you, we will send a reminder.", "Cảm ơn chị, chúng tôi sẽ gửi nhắc nhở."),
        # 29 volunteer sign-up sheet
        ("Ms Lan", "What number should go on the volunteer sign-up sheet?", "Nên ghi số nào vào tờ đăng ký tình nguyện viên vậy?"),
        ("Mr Leopold", "Put zero six two three four five six seven eight, please.", "Vui lòng ghi không sáu hai ba bốn năm sáu bảy tám."),
        ("Ms Lan", "Perfect, that number is on the sheet, welcome aboard.", "Hoàn hảo, số đó đã ghi vào tờ đăng ký, chào mừng anh."),
        # 30 parcel delivery form
        ("Mr Leopold", "What number should the courier call before delivering this?", "Nhân viên chuyển phát nên gọi số nào trước khi giao vậy?"),
        ("Ms Lan", "Call zero six three four five six seven eight nine, please.", "Vui lòng gọi không sáu ba bốn năm sáu bảy tám chín."),
        ("Mr Leopold", "Thank you, noted on the label.", "Cảm ơn chị, đã ghi vào nhãn."),
        # 31 subscription account
        ("Ms Lan", "What number should be linked to this subscription account?", "Nên liên kết số nào với tài khoản đăng ký gói dịch vụ này vậy?"),
        ("Mr Leopold", "Link zero six four five six seven eight nine zero, please.", "Vui lòng liên kết không sáu bốn năm sáu bảy tám chín không."),
        ("Ms Lan", "Great, the subscription is now tied to that number.", "Tốt, gói đăng ký giờ đã gắn với số đó."),
        # 32 insurance policy form
        ("Mr Leopold", "What number should go on your insurance policy form?", "Nên ghi số nào vào mẫu đơn hợp đồng bảo hiểm của chị vậy?"),
        ("Ms Lan", "Put zero six five six seven eight nine zero one, please.", "Vui lòng ghi không sáu năm sáu bảy tám chín không một."),
        ("Mr Leopold", "Thank you, your policy is now on file.", "Cảm ơn chị, hợp đồng của chị đã được ghi nhận."),
        # 33 driving lesson booking
        ("Ms Lan", "What number should the driving instructor call to confirm?", "Giáo viên dạy lái nên gọi số nào để xác nhận vậy?"),
        ("Mr Leopold", "Call zero six six seven eight nine zero one two, please.", "Vui lòng gọi không sáu sáu bảy tám chín không một hai."),
        ("Ms Lan", "Thank you, your first lesson is confirmed.", "Cảm ơn anh, buổi học đầu tiên đã xác nhận."),
        # 34 rental application
        ("Mr Leopold", "What number should go on this rental application?", "Nên ghi số nào vào đơn xin thuê nhà này vậy?"),
        ("Ms Lan", "Put zero six seven eight nine zero one two three, please.", "Vui lòng ghi không sáu bảy tám chín không một hai ba."),
        ("Mr Leopold", "Thank you, we will review it this week.", "Cảm ơn chị, tuần này chúng tôi sẽ xem xét."),
        # 35 conference registration
        ("Ms Lan", "What number should be on your conference registration badge?", "Nên ghi số nào vào thẻ đăng ký hội nghị của anh vậy?"),
        ("Mr Leopold", "Put zero six eight nine zero one two three four, please.", "Vui lòng ghi không sáu tám chín không một hai ba bốn."),
        ("Ms Lan", "Thank you for always double-checking the digits.", "Cảm ơn anh vì luôn kiểm tra kỹ từng chữ số."),
        # 36 closing on why sharing a number clearly matters
        ("Mr Leopold", "Why does reading a phone number out clearly matter so much anyway?", "Vậy vì sao đọc số điện thoại rõ ràng lại quan trọng đến thế?"),
        ("Ms Lan", "It avoids so many mix-ups when someone tries to call back.", "Nó giúp tránh nhiều nhầm lẫn khi ai đó cố gọi lại."),
        ("Mr Leopold", "That is very true, I will always speak slowly from now on.", "Đúng vậy thật, từ giờ tôi sẽ luôn nói chậm rãi."),
    ],
}
