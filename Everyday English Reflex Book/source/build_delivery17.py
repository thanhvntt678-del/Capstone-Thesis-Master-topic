# -*- coding: utf-8 -*-
"""Builds Delivery #17 (Lessons 0136 onward) — same pattern as build_delivery16.py:
no pre-approved base docx, cross-checks duplicates against the WHOLE book so far
(Deliveries #1-#16 plus whatever's in this delivery)."""
import sys
sys.path.insert(0, '.')
import docx
...
LESSONS_DELIVERY17 = [LESSON_0136, LESSON_0137, LESSON_0138, LESSON_0139, LESSON_0140, LESSON_0141, LESSON_0142, LESSON_0143, LESSON_0144]  # 9 lessons closes near ~37 pages; 0145-0149 open Delivery #18

def main():
    doc = docx.Document()
    new_section_setup(doc)

    for idx, lesson in enumerate(LESSONS_DELIVERY17):
        if idx != 0:
            doc.add_page_break()
        add_title_block(doc)
    ...
    lesson_range = f"{LESSONS_DELIVERY17[0]['lesson_id']}-{LESSONS_DELIVERY17[-1]['lesson_id']}"
    ...
    print("\n=== PER-LESSON QC (Delivery #17) ===")
    for lesson in LESSONS_DELIVERY17:
        r = qc_report(lesson)
    ...
    WHOLE_BOOK = [lesson0001] + DELIVERY1 + DELIVERY2 + DELIVERY3 + DELIVERY4 + DELIVERY5 + DELIVERY6 + DELIVERY7 + DELIVERY8 + DELIVERY9 + DELIVERY10 + DELIVERY11 + DELIVERY12 + DELIVERY13 + DELIVERY14 + DELIVERY15 + DELIVERY16 + LESSONS_DELIVERY17
    dups = cross_lesson_duplicate_check(WHOLE_BOOK)
    print(f"\nCross-lesson duplicate English lines (whole book, 0001 through {LESSONS_DELIVERY17[-1]['lesson_id']}):", dups)

    total_words = sum(lesson_word_count(l) for l in LESSONS_DELIVERY17)
    total_turns = sum(len(l['turns']) for l in LESSONS_DELIVERY17) + len(LESSONS_DELIVERY17)
    est_pages = total_turns / (133 / 5)
    print(f"\nTotal English learning words, Delivery #17 ({lesson_range}):", total_words)
    print(f"Structural page estimate for Delivery #17 so far: {est_pages:.1f} pages")
