# -*- coding: utf-8 -*-
"""Builds Delivery #6 (Lessons 0040-0047) — same pattern as build_delivery5.py:
no pre-approved base docx, cross-checks duplicates against the WHOLE book so far
(Deliveries #1-#5 plus whatever's in this delivery)."""
import sys
sys.path.insert(0, '.')
import docx
...
LESSONS_DELIVERY6 = [LESSON_0040, LESSON_0041, LESSON_0042, LESSON_0043, LESSON_0044, LESSON_0045, LESSON_0046, LESSON_0047]  # closing this delivery at 0047

def main():
    doc = docx.Document()
    new_section_setup(doc)

    for idx, lesson in enumerate(LESSONS_DELIVERY6):
        if idx != 0:
            doc.add_page_break()
        add_title_block(doc)
    ...
    lesson_range = f"{LESSONS_DELIVERY6[0]['lesson_id']}-{LESSONS_DELIVERY6[-1]['lesson_id']}"
    ...
    print("\n=== PER-LESSON QC (Delivery #6) ===")
    for lesson in LESSONS_DELIVERY6:
        r = qc_report(lesson)
    ...
    WHOLE_BOOK = [lesson0001] + DELIVERY1 + DELIVERY2 + DELIVERY3 + DELIVERY4 + DELIVERY5 + LESSONS_DELIVERY6
    dups = cross_lesson_duplicate_check(WHOLE_BOOK)
    print(f"\nCross-lesson duplicate English lines (whole book, 0001 through {LESSONS_DELIVERY6[-1]['lesson_id']}):", dups)

    total_words = sum(lesson_word_count(l) for l in LESSONS_DELIVERY6)
    total_turns = sum(len(l['turns']) for l in LESSONS_DELIVERY6) + len(LESSONS_DELIVERY6)
    est_pages = total_turns / (133 / 5)
    print(f"\nTotal English learning words, Delivery #6 ({lesson_range}):", total_words)
    print(f"Structural page estimate for Delivery #6 so far: {est_pages:.1f} pages")
