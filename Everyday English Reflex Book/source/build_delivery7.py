# -*- coding: utf-8 -*-
"""Builds Delivery #7 (Lessons 0048 onward) — same pattern as build_delivery6.py:
no pre-approved base docx, cross-checks duplicates against the WHOLE book so far
(Deliveries #1-#6 plus whatever's in this delivery)."""
import sys
sys.path.insert(0, '.')
import docx
...
LESSONS_DELIVERY7 = [LESSON_0048, LESSON_0049, LESSON_0050, LESSON_0051, LESSON_0052, LESSON_0053, LESSON_0054, LESSON_0055]  # closing this delivery at 0055

def main():
    doc = docx.Document()
    new_section_setup(doc)

    for idx, lesson in enumerate(LESSONS_DELIVERY7):
        if idx != 0:
            doc.add_page_break()
        add_title_block(doc)
    ...
    lesson_range = f"{LESSONS_DELIVERY7[0]['lesson_id']}-{LESSONS_DELIVERY7[-1]['lesson_id']}"
    ...
    print("\n=== PER-LESSON QC (Delivery #7) ===")
    for lesson in LESSONS_DELIVERY7:
        r = qc_report(lesson)
    ...
    WHOLE_BOOK = [lesson0001] + DELIVERY1 + DELIVERY2 + DELIVERY3 + DELIVERY4 + DELIVERY5 + DELIVERY6 + LESSONS_DELIVERY7
    dups = cross_lesson_duplicate_check(WHOLE_BOOK)
    print(f"\nCross-lesson duplicate English lines (whole book, 0001 through {LESSONS_DELIVERY7[-1]['lesson_id']}):", dups)

    total_words = sum(lesson_word_count(l) for l in LESSONS_DELIVERY7)
    total_turns = sum(len(l['turns']) for l in LESSONS_DELIVERY7) + len(LESSONS_DELIVERY7)
    est_pages = total_turns / (133 / 5)
    print(f"\nTotal English learning words, Delivery #7 ({lesson_range}):", total_words)
    print(f"Structural page estimate for Delivery #7 so far: {est_pages:.1f} pages")
