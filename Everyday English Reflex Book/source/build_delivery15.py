# -*- coding: utf-8 -*-
"""Builds Delivery #15 (Lessons 0118 onward) — same pattern as build_delivery14.py:
...
LESSONS_DELIVERY15 = [LESSON_0118, LESSON_0119, LESSON_0120, LESSON_0121, LESSON_0122, LESSON_0123, LESSON_0124, LESSON_0125, LESSON_0126]  # append more here as written; closes near ~37 pages

def main():
    doc = docx.Document()
    new_section_setup(doc)

    for idx, lesson in enumerate(LESSONS_DELIVERY15):
        if idx != 0:
            doc.add_page_break()
        add_title_block(doc)
    ...
    lesson_range = f"{LESSONS_DELIVERY15[0]['lesson_id']}-{LESSONS_DELIVERY15[-1]['lesson_id']}"
    ...
    print("\n=== PER-LESSON QC (Delivery #15) ===")
    for lesson in LESSONS_DELIVERY15:
        r = qc_report(lesson)
    ...
    WHOLE_BOOK = [lesson0001] + DELIVERY1 + DELIVERY2 + DELIVERY3 + DELIVERY4 + DELIVERY5 + DELIVERY6 + DELIVERY7 + DELIVERY8 + DELIVERY9 + DELIVERY10 + DELIVERY11 + DELIVERY12 + DELIVERY13 + DELIVERY14 + LESSONS_DELIVERY15
    dups = cross_lesson_duplicate_check(WHOLE_BOOK)
    print(f"\nCross-lesson duplicate English lines (whole book, 0001 through {LESSONS_DELIVERY15[-1]['lesson_id']}):", dups)

    total_words = sum(lesson_word_count(l) for l in LESSONS_DELIVERY15)
    total_turns = sum(len(l['turns']) for l in LESSONS_DELIVERY15) + len(LESSONS_DELIVERY15)
    est_pages = total_turns / (133 / 5)
    print(f"\nTotal English learning words, Delivery #15 ({lesson_range}):", total_words)
    print(f"Structural page estimate for Delivery #15 so far: {est_pages:.1f} pages")
