# -*- coding: utf-8 -*-
"""Builds Delivery #16 (Lessons 0127 onward) — same pattern as build_delivery15.py:
...
LESSONS_DELIVERY16 = [LESSON_0127, LESSON_0128, LESSON_0129, LESSON_0130, LESSON_0131, LESSON_0132, LESSON_0133, LESSON_0134, LESSON_0135]  # append more here as written; closes near ~37 pages

def main():
    doc = docx.Document()
    new_section_setup(doc)

    for idx, lesson in enumerate(LESSONS_DELIVERY16):
        if idx != 0:
            doc.add_page_break()
        add_title_block(doc)
    ...
    lesson_range = f"{LESSONS_DELIVERY16[0]['lesson_id']}-{LESSONS_DELIVERY16[-1]['lesson_id']}"
    ...
    print("\n=== PER-LESSON QC (Delivery #16) ===")
    for lesson in LESSONS_DELIVERY16:
        r = qc_report(lesson)
    ...
    WHOLE_BOOK = [lesson0001] + DELIVERY1 + DELIVERY2 + DELIVERY3 + DELIVERY4 + DELIVERY5 + DELIVERY6 + DELIVERY7 + DELIVERY8 + DELIVERY9 + DELIVERY10 + DELIVERY11 + DELIVERY12 + DELIVERY13 + DELIVERY14 + DELIVERY15 + LESSONS_DELIVERY16
    dups = cross_lesson_duplicate_check(WHOLE_BOOK)
    print(f"\nCross-lesson duplicate English lines (whole book, 0001 through {LESSONS_DELIVERY16[-1]['lesson_id']}):", dups)

    total_words = sum(lesson_word_count(l) for l in LESSONS_DELIVERY16)
    total_turns = sum(len(l['turns']) for l in LESSONS_DELIVERY16) + len(LESSONS_DELIVERY16)
    est_pages = total_turns / (133 / 5)
    print(f"\nTotal English learning words, Delivery #16 ({lesson_range}):", total_words)
    print(f"Structural page estimate for Delivery #16 so far: {est_pages:.1f} pages")
