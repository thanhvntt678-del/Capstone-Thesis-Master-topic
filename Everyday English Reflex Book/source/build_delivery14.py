# -*- coding: utf-8 -*-
"""Builds Delivery #14 (Lessons 0109 onward) — same pattern as build_delivery13.py:
...
LESSONS_DELIVERY14 = [LESSON_0109, LESSON_0110, LESSON_0111, LESSON_0112, LESSON_0113, LESSON_0114, LESSON_0115, LESSON_0116, LESSON_0117]  # append more here as written; closes near ~37 pages

def main():
    doc = docx.Document()
    new_section_setup(doc)

    for idx, lesson in enumerate(LESSONS_DELIVERY14):
        if idx != 0:
            doc.add_page_break()
        add_title_block(doc)
    ...
    lesson_range = f"{LESSONS_DELIVERY14[0]['lesson_id']}-{LESSONS_DELIVERY14[-1]['lesson_id']}"
    ...
    print("\n=== PER-LESSON QC (Delivery #14) ===")
    for lesson in LESSONS_DELIVERY14:
        r = qc_report(lesson)
    ...
    WHOLE_BOOK = [lesson0001] + DELIVERY1 + DELIVERY2 + DELIVERY3 + DELIVERY4 + DELIVERY5 + DELIVERY6 + DELIVERY7 + DELIVERY8 + DELIVERY9 + DELIVERY10 + DELIVERY11 + DELIVERY12 + DELIVERY13 + LESSONS_DELIVERY14
    dups = cross_lesson_duplicate_check(WHOLE_BOOK)
    print(f"\nCross-lesson duplicate English lines (whole book, 0001 through {LESSONS_DELIVERY14[-1]['lesson_id']}):", dups)

    total_words = sum(lesson_word_count(l) for l in LESSONS_DELIVERY14)
    total_turns = sum(len(l['turns']) for l in LESSONS_DELIVERY14) + len(LESSONS_DELIVERY14)
    est_pages = total_turns / (133 / 5)
    print(f"\nTotal English learning words, Delivery #14 ({lesson_range}):", total_words)
    print(f"Structural page estimate for Delivery #14 so far: {est_pages:.1f} pages")
